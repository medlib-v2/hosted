class Hosted
  ...
  def Hosted.configure(config, settings)
    # Set The VM Provider
    ...
    ["vmware_fusion", "vmware_workstation.rb"].each do |vmware|
      config.vm.provider vmware do |v|
        v.vmx["displayName"] = settings["name"] ||= "hosted"
    ...
    if settings.include? 'folders'
      settings["folders"].each do |folder|
        if File.exists? File.expand_path(folder["map"])
          ...
          config.vm.provision "shell" do |s|
            s.inline = ">&2 echo \"Unable to mount one of your folders. Please check your folders in Settings.yaml\""
          end
        end
      end
    end
    ...
    if settings.has_key?("variables")
      settings["variables"].each do |var|
        ...
        config.vm.provision "shell" do |s|
            s.inline = "echo \"\n# Set Hosted Environment Variable\nexport $1=$2\" >> /home/vagrant/.profile"
            s.args = [var["key"], var["value"]]
        end
      end
